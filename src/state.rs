...
///
/// The credential fields (`claude_token`, `copilot_token`, `openrouter_key`,
/// `gemini_key`, `http_proxy`) are encrypted on disk (DPAPI on Windows, ChaCha20-Poly1305 on
...
pub struct AppState {
    ...
    /// Claude OAuth token (sk-ant-oat01-…), generated by the user with
    /// `claude setup-token` and pasted into the settings panel.
